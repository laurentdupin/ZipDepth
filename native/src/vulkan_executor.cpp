...
VulkanExecutor::Tensor VulkanExecutor::rep(const Tensor&i,const std::string&p,std::uint32_t stride){
    ...
    Tensor a=bn(conv(i,p+".branch_3x3.0.weight",nullptr,stride,1),p+".branch_3x3.1",false);Tensor b=bn(conv(i,p+".branch_1x1.0.weight",nullptr,stride,0),p+".branch_1x1.1",false);Tensor o=add(a,b);if(stride==1&&i.channels==o.channels)o=add(o,i);Tensor r=make(o.channels,o.height,o.width);operators_.activation(r.buffer,o.buffer,checked(elements(o.channels,o.height,o.width)),1);return r;}
...
VulkanExecutor::Tensor VulkanExecutor::infer_device(midas_native::VulkanBuffer input,std::uint32_t width,std::uint32_t height){
    ...
    context_.batch([&]{
        ...
        output = {
            ...
            width,
#if defined(__ANDROID__)
            context_.create_host_buffer(elements(1, height, width) * sizeof(float))
#else
            context_.create_device_buffer(elements(1, height, width) * sizeof(float))
#endif
        };
