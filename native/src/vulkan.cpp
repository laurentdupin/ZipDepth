...
VulkanContext::VulkanContext(
    ...
    const bool has_external_semaphore_fd = has_extension(
        extensions, VK_KHR_EXTERNAL_SEMAPHORE_FD_EXTENSION_NAME);
    const bool has_dma_buf = has_extension(extensions, VK_EXT_EXTERNAL_MEMORY_DMA_BUF_EXTENSION_NAME);
    const bool has_foreign = has_extension(extensions, VK_EXT_QUEUE_FAMILY_FOREIGN_EXTENSION_NAME);
    if (has_external_memory_fd && has_drm_format_modifier && has_dma_buf && has_foreign) {
    ...
    external_capabilities_.dma_buf_import =
        has_external_memory_fd && has_drm_format_modifier && has_dma_buf && has_foreign &&
...
VulkanImage VulkanContext::import_dma_buf(
    ...
    const auto imported =
        inferbridge::native_harness::import_linux_dma_buf_vulkan(
            physical_device_, device_, get_memory_fd_properties_, source, format, usage,
